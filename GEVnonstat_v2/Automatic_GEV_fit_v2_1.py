    ...
    def AutoAdjust(self, maxiter: int = 100, plot: bool = False):
        ...
        if self.covariates is not None:
            ...
            for i in tqdm(range(self.niter_harm +1, self.niter_harm+3*auxnind_loc+1), desc="Covariate Iteration"):
                ...
                self.AICiter[i] = self._AIC(-loglikeobj, 2+aux_neps0[i-self.niter_harm-1]+2*nmu+2*npsi+2*neps+ntend_loc+nind_loc+ntend_sc+nind_sc+nind_sh)
                ...
                if self.AICiter[i] < self.AICini:
                    ...
                    if maximo_loc > maximo_sc and maximo_loc > maximo_sh:
                        list_loc = list_loc[:-1]
                        varphi = varphi[:-1]
                        nind_loc -= 1 
                    elif maximo_sc > maximo_loc and maximo_sc > maximo_sh:
                        list_sc = list_sc[:-1]
                        varphi2 = varphi2[:-1]
                        nind_sc -= 1
                    else:
