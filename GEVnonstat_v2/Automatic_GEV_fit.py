...
class GEVnonstat:
    ...
    def AutoAdjust(self, maxiter = 100, plot = False):
        ...
        if self.covariates is not None:
            self.maxiter = maxiter +2*self.covariates.shape[1]
        else:
        ...
        if self.harm:
            ...
            beta0, beta, alpha0, alpha, gamma0, gamma, betaT, varphi, betaT2, varphi2, loglikeobj, grad, hessian, _ = self._optimize_parameters(nmu, npsi, neps)
            self.AICiter[0] = self._AIC(-loglikeobj, 2 + self.neps0 + 2*nmu + 2*npsi + 2*neps + nind_loc + ntend_loc + nind_sc + ntend_sc)
        ...
        if self.trends:
            ...
            pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], np.zeros(ntend_loc), self.varphi, popt[1+2*self.nmu:2+2*self.nmu+2*self.npsi], self.varphi2, np.zeros(self.neps0), np.zeros(2*self.neps)])
            ...
            if self.AICiter[self.nit+1] > self.AICini:
                ...
                pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], self.varphi, popt[2+2*self.nmu-1:2+2*self.nmu+2*self.npsi], self.varphi2, np.zeros(self.neps0), np.zeros(2*self.neps)])
                beta0,beta,alpha0,alpha,gamma0,gamma,betaT,varphi,betaT2,varphi2,loglikeobj,grad,hessian,_ = self._optimize_parameters(self.nmu,self.npsi,self.neps,betaT,self.covariates[:,self.list_loc],np.zeros(nind_loc),betaT2,self.covariates[:,self.list_sc],np.zeros(nind_sc),pini)
            ...
            ntend_sc = 1
            betaT2 = np.array([0])
            pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], np.zeros(ntend_loc),self.varphi, popt[2+2*self.nmu-1:2+2*self.nmu+2*self.npsi], np.zeros(ntend_sc), self.varphi2, np.zeros(self.neps0), np.zeros(2*self.neps)])
            ...
            if self.AICiter[self.nit+2] > self.AICini:
                ...
                pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], np.zeros(ntend_loc), self.varphi, popt[2+2*self.nmu-1:2+2*self.nmu+2*self.npsi], self.varphi2, np.zeros(self.neps0), np.zeros(2*self.neps)])
                beta0,beta,alpha0,alpha,gamma0,gamma,betaT,varphi,betaT2,varphi2,loglikeobj,grad,hessian,_ = self._optimize_parameters(self.nmu,self.npsi,self.neps,self.betaT,self.covariates[:,self.list_loc],np.zeros(nind_loc),betaT2,self.covariates[:,self.list_sc],np.zeros(nind_sc),pini)
            ...
            pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], self.varphi, popt[1+2*self.nmu:2+2*self.nmu+2*self.npsi], self.varphi2, np.zeros(self.neps0), np.zeros(2*self.neps)])
            beta0,beta,alpha0,alpha,gamma0,gamma,betaT,varphi,betaT2,varphi2,loglikeobj,grad,hessian,_ = self._optimize_parameters(self.nmu,self.npsi,self.neps,None,self.covariates[:,self.list_loc],np.zeros(nind_loc),None,self.covariates[:,self.list_sc],np.zeros(nind_sc),pini)
        ...
        if all(np.abs(epst1) < 1e-8):
            ...
            pini = np.concatenate([np.array([self.beta0]), popt[1:1+2*self.nmu], np.zeros(ntend_loc), self.varphi, popt[1+2*self.nmu:2+2*self.nmu+2*self.npsi], np.zeros(ntend_sc), self.varphi2])
            beta0,beta,alpha0,alpha,gamma0,gamma,betaT,varphi,betaT2,varphi2,loglikeobj,grad,hessian,_ = self._optimize_parameters(self.nmu,self.npsi,self.neps,self.betaT,self.covariates[:,self.list_loc],np.zeros(nind_loc),self.betaT2,self.covariates[:,self.list_sc],np.zeros(nind_sc),pini)
    ...
    def _aggquantile(self, q, t0, t1, beta0=None,beta=None,alpha0=None,alpha=None,gamma0=None,gamma=None,betaT=None,betaT2=None,varphi=None,varphi2=None):
        ...
        for il in range(m):
            ...
            while err > 1e-4 and iter < 1000:
                zqold = zq
                integ = quad(lambda x: self._fzeroquanint(x,zqold,q[il],cov_locint,cov_scint,beta0,beta,alpha0,alpha,gamma0,gamma,betaT,betaT2,varphi,varphi2),t0[il],t1[il])[0]
                integ += np.log(q[il])/12
                dint = quad(lambda x: self._fzeroderiquanint(x,zqold,q[il],cov_locint,cov_scint,beta0,beta,alpha0,alpha,gamma0,gamma,betaT,betaT2,varphi,varphi2),t0[il],t1[il])[0]
                zq += -integ/dint
                if np.abs(zq) > 1e-5:
