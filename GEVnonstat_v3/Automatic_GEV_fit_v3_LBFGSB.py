...
class GEVnonstat:
    ...
    def __init__(self, xt, t, kt=None, covariates: dict=None, ejemplo: str=None, quanval: float=0.95, trends: bool=False, harm: bool = True):
        """
        ...
        self.harm = harm            # True by default (if harmonic part should be introduced)            
    ...
    def AutoAdjust(self, maxiter = 100, plot = False):
        ...
        if self.covariates is not None:
            ...
            for i in tqdm(range(self.niter_harm +1, self.niter_harm+2*auxnind_loc+1), desc="Covariate Iteration"):
                ...
                _, auxJx, _ = self._loglikelihood(beta0,beta,alpha0,alpha,gamma0,gamma,betaT,auxvarphi_loc,betaT2,auxvarphi_sc,self.covariates,self.covariates)
                auxp = np.concatenate([np.atleast_1d(value) for value in [beta0, beta, betaT, auxvarphi_loc, alpha0, alpha, betaT2, auxvarphi_sc, gamma0, gamma] if value is not None])   # Concatenate only if the value is not None
                auxHxx = self.compute_numerical_hessian(auxp,2*nmu,2*npsi,2*neps,ntend_loc,self.covariates,ntend_sc,self.covariates)
...
### 
    ...
    def _optimize_parameters(self, nmu, npsi, neps, betaT=None, covariates_loc=None, varphi=None, betaT2=None, covariates_sc=None, varphi2=None, pini = None):
        ...
        result = minimize(
            ...
            method = "L-BFGS-B" # L-BFGS-B, TNC, SLSQP, trust-constr
        )
    ...
    def _auxmin_loglikelihood(self, p, nmu, npsi, neps, ntend_loc=None, covariates_loc=None, ntend_sc=None, covariates_sc=None):
        ...
        #if any(xn[posG] < -1e8):
        #    print("xt", self.xt[posG])
        #    print("mut",mut[posG])
        #    print("xt-mut", self.xt[posG]-mut[posG])
        #    print("psit",psit1[posG])
        #    print("xn", xn[posG])
        #    print("alpha0", p[1+nmu+ntend_loc+nind_loc])
        #    print("alpha", p[2+nmu+ntend_loc+nind_loc:2+nmu+ntend_loc+nind_loc+npsi])
        #    print("betaT2", p[2+nmu+ntend_loc+nind_loc+npsi])
        #    #print(self._parametro(p[1+nmu+ntend_loc+nind_loc], p[2+nmu+ntend_loc+nind_loc:2+nmu+ntend_loc+nind_loc+npsi], np.array(p[2+nmu+ntend_loc+nind_loc+npsi]), covariates_sc, p[2+nmu+ntend_loc+nind_loc+npsi+ntend_sc:2+nmu+ntend_loc+nind_loc+npsi+ntend_sc+nind_sc]))
        #    #print("alpha0",p[1+nmu+ntend_loc+nind_loc])
        #    #print("alpha", p[2+nmu+ntend_loc+nind_loc:2+nmu+ntend_loc+nind_loc+npsi])
        #    #print("betaT2", np.array(p[2+nmu+ntend_loc+nind_loc+npsi]))
        #    #print("varphi2", p[2+nmu+ntend_loc+nind_loc+npsi+ntend_sc:2+nmu+ntend_loc+nind_loc+npsi+ntend_sc+nind_sc])
